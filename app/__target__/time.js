// Transcrypt'ed from Python, 2023-12-27 16:24:53
...
export var __adapt__ = function (request) {
	__language = __k__ (__k__ (request.headers, 'accept-language').py_split (','), 0);
};
...
for (var i = 0; i < 7; i++) {
	for (var [l, s] of __i__ (tuple ([tuple ([__weekdays, 'short']), tuple ([__weekdays_long, 'long'])]))) {
...
for (var i = 0; i < 12; i++) {
	for (var [l, s] of __i__ (tuple ([tuple ([__months, 'short']), tuple ([__months_long, 'long'])]))) {
...
export var _daylight_in_effect = function (t) {
	var jj = __jan_jun_tz (t);
	if (min (__k__ (jj, 0), __k__ (jj, 1)) == t.getTimezoneOffset ()) {
		return 1;
	}
	return 0;
};
export var _timezone = function (t) {
	var jj = __jan_jun_tz (t);
	return max (__k__ (jj, 0), __k__ (jj, 1));
};
export var __tzn = function (t) {
	try {
		return __k__ (__k__ (str (t).py_split ('('), 1).py_split (')'), 0);
	}
	catch (__except0__) {
		return 'n.a.';
	}
...
if (!(_daylight_in_effect (__now))) {
	...
	var altzone = (altzone == __k__ (_jj, 1) ? __k__ (_jj, 0) : __k__ (_jj, 1));
}
...
export var mktime = function (t) {
	var d = new Date (__k__ (t, 0), __k__ (t, 1) - 1, __k__ (t, 2), __k__ (t, 3), __k__ (t, 4), __k__ (t, 5), 0);
	return (d - 0) / 1000;
};
...
export var strptime = function (string, format) {
	...
	while (ts) {
		...
		if (sep == '') {
			...
			if (d) {
				...
				if (d == 'Y') {
					...
					var l = len (__k__ (__weekdays, 0));
				}
				else if (d == 'A') {
					var l = len (__k__ (__weekdays_long, 0));
				}
				else if (d == 'b') {
					var l = len (__k__ (__months, 0));
				}
		...
		var __left0__ = tuple ([__k__ (lv, 1), __k__ (lv, 0)]);
		var ts = __left0__ [0];
	...
	for (var [d, v] of __i__ (dir_val.py_items ())) {
	...
	__date.setUTCFullYear (__k__ (t, 0));
	__date.setUTCMonth (__k__ (t, 1) - 1);
	__date.setUTCDate (__k__ (t, 2));
	__date.setUTCHours (__k__ (t, 3));
...
export var strftime = function (format, t) {
	...
	var f = format;
	for (var d of __i__ (__lu.py_keys ())) {
		var k = '%' + d;
		if (!(__in__ (k, f))) {
			continue;
		}
		var v = zf2 (__k__ (t, __k__ (__lu, d)));
		var f = f.py_replace (k, v);
	}
	for (var [d, l, pos] of __i__ (tuple ([tuple (['b', __months, 1]), tuple (['B', __months_long, 1]), tuple (['a', __weekdays, 6]), tuple (['A', __weekdays_long, 6])]))) {
		var p = __k__ (t, pos);
		if (pos == 1) {
			var p = p - 1;
		}
		var v = __k__ (l, p).capitalize ();
	...
	if (__in__ ('%y', f)) {
		var f = f.py_replace ('%y', str (__k__ (t, 0)).__getslice__ (-(2), null, 1));
	}
	if (__in__ ('%I', f)) {
		var v = __k__ (t, 3);
		if (v == 0) {
