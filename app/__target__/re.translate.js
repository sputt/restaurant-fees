...
export var getCaptureGroup = function (groupInfo, namedGroups, groupRef) {
	...
	var search = 0;
	for (var group of __i__ (groupInfo)) {
...
export var splitIfElse = function (tokens, namedGroups) {
	...
	for (var group of __i__ (groupInfo)) {
		if (__t__ (group.klass == '(?<')) {
			...
			var captureGroupModifier = __k__ (tokens, captureGroup.end + 1);
			if (__t__ (__t__ (__in__ (captureGroupModifier.py_name, ['?', '*'])) || captureGroupModifier.py_name.startswith ('{0,'))) {
				if (__t__ (captureGroupModifier.py_name == '?')) {
					...
					__k__ (iff, captureGroup.end + 1).py_name.__setslice__ (0, 3, null, '{1,');
				}
				els [captureGroup.end + 1] = null;
				var hasElse = false;
				for (var idx = conStart; idx < conEnd; idx++) {
					if (__t__ (__k__ (tokens, idx).py_name == '|')) {
						var hasElse = true;
						els.py_pop (conEnd);
				...
				var pastIff = false;
				for (var idx = conStart; idx < conEnd; idx++) {
					if (__t__ (__k__ (iff, idx).py_name == '|')) {
	...
	var allVariants = [];
	for (var variant of __i__ (variants)) {
...
export var Token =  __class__ ('Token', [object], {
	...
	get resolve () {return __get__ (this, function (self) {
		var paras = '';
		for (var para of __i__ (self.paras)) {
			paras += str (para);
		}
...
export var shiftReduce = function (stack, queue, namedGroups, flags) {
	...
	var s0 = (__t__ (len (stack) > 0) ? __k__ (stack, high) : Token (''));
	var s1 = (__t__ (len (stack) > 1) ? __k__ (stack, high - 1) : Token (''));
	if (__t__ (VERBOSE)) {
		for (var token of __i__ (stack)) {
	...
	if (__t__ (s1.py_name == '\\')) {
		...
		if (__t__ (s0.py_name == ')')) {
			stack.__setslice__ (-__t__ ((2)), null, null, [Token ('\\' + str (__k__ (namedGroups, __k__ (s1.paras, 0))))]);
		}
