...
export var Match =  __class__ ('Match', [object], {
	...
	get __init__ () {return __get__ (this, function (self, mObj, string, pos, endpos, rObj, namedGroups) {
		...
		for (var [index, match] of __i__ (enumerate (mObj))) {
			mObj [index] = (__k__ (mObj, index) == undefined ? null : __k__ (mObj, index));
		}
		self._obj = mObj;
		self._pos = pos;
		self._endpos = endpos;
		...
		if (self._namedGroups !== null) {
			self._lastgroup = __k__ (self._namedGroups, self._lastindex);
		}
		else {
			...
			for (var index of __i__ (args)) {
				if (py_typeof (index) === str) {
					if (self._namedGroups !== null) {
						...
						ret.append (__k__ (self._obj, __k__ (self._namedGroups, index)));
					}
					else {
					...
					ret.append (__k__ (self._obj, index));
				}
			}
		}
		else {
			ret.append (__k__ (self._obj, 0));
		}
		if (len (ret) == 1) {
			return __k__ (ret, 0);
		}
	...
	get groupdict () {return __get__ (this, function (self, py_default) {
		...
		if (self._namedGroups !== null) {
			var ret = dict ({});
			for (var [gName, gId] of __i__ (self._namedGroups.py_items ())) {
				var value = __k__ (self._obj, gId);
	...
	get start () {return __get__ (this, function (self, group) {
		...
		if (py_typeof (group) === str) {
			if (self._namedGroups !== null) {
				...
				var gId = __k__ (self._namedGroups, group);
			}
			else {
				var __except0__ = NotImplementedError ('No NamedGroups Available');
				__except0__.__cause__ = null;
		...
		else if (__k__ (self._obj, gId) !== null) {
			var r = compile (escape (__k__ (self._obj, gId)), self._re.flags);
			var m = r.search (__k__ (self._obj, 0));
			if (m) {
	...
	get end () {return __get__ (this, function (self, group) {
		...
		if (gId == 0) {
			return self._obj.index + len (__k__ (self._obj, 0));
		}
		else if (__k__ (self._obj, gId) !== null) {
			var r = compile (escape (__k__ (self._obj, gId)), self._re.flags);
			var m = r.search (__k__ (self._obj, 0));
...
export var Regex =  __class__ ('Regex', [object], {
	...
	get _convertFlags () {return __get__ (this, function (self, flags) {
		...
		var ret = ''.join ((function () {
			var __accu0__ = [];
			for (var x of __i__ (bitmaps)) {
				if ((x [0] & flags) > 0) {
					__accu0__.append (__k__ (x, 1));
				}
			}
			return __accu0__;
		}) ());
	...
	get fullmatch () {return __get__ (this, function (self, string, pos, endpos) {
		...
		var m = rObj.exec (target);
		if (m) {
			var obsEndPos = m.index + len (__k__ (m, 0));
	...
	get py_split () {return __get__ (this, function (self, string, maxsplit) {
		...
		if (maxsplit == 0) {
			...
			if (lastM !== null) {
				var endPos = lastM.index + len (__k__ (lastM, 0));
